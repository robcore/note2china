...
struct tsp_callbacks {
	void (*inform_charger)(struct tsp_callbacks *tsp_cb, bool mode, int mode2);
};
...
static void mms_set_noise_mode(struct mms_ts_info *info, int index)
{
	struct i2c_client *client = info->client;

	dev_notice(&client->dev, "%s, id=%d (ta,noise,wake,clear)\n", __func__, index);

	if (!(info->enabled)) 
		return;

	if(index == N_WAKEUP)
		if(info->noise_mode) index = N_NOISE;
	...
	if (info->ta_status) {
		...
		i2c_smbus_write_byte_data(info->client, 0x33, 0x1);
				...
				dev_notice(&client->dev, "TA disconnect!!!, noise:%d\n",info->noise_mode );
	...
	case N_NOISE :
		{
			dev_notice(&client->dev, "noise mode enter, t:%d, n:%d!!!\n", info->ta_status, info->noise_mode);
		if (info->ta_status) {
	...
	case N_WAKEUP :
		{
			dev_notice(&client->dev, "noise mode wakeup, t:%d, n:%d!!!\n", info->ta_status, info->noise_mode);
			if (info->ta_status) {
	...
	case N_CLEAR :
		{
			dev_notice(&client->dev, "noise mode clear !!!\n");
			//i2c_smbus_write_byte_data(info->client, 0x30, 0x2);		//  noise mode 
			//i2c_smbus_write_byte_data(info->client, 0x32, 0x2);		//  wakeup mode
			//i2c_smbus_write_byte_data(info->client, 0x33, 0x2);		// ta status
		}
		break;
	}
...
static void reset_mms_ts(struct mms_ts_info *info)
{
	...
	info->enabled = true;
	mms_set_noise_mode(info, N_WAKEUP);
...
static void melfas_ta_cb(struct tsp_callbacks *cb, bool ta_status, int mode)
{
	struct mms_ts_info *info =
			container_of(cb, struct mms_ts_info, callbacks);
	struct i2c_client *client = info->client;
	static int ta_mode = 0;
	static int cam_mode = 0;

	dev_notice(&client->dev, "%s, mode %d (%d, %d) \n", __func__, ta_mode, ta_status, mode);

	if(mode==1){		// tsp mode
		ta_mode = ta_status;
	}else if(mode==2){	// cam mode
		cam_mode = ta_status;
		}

	info->ta_status = (ta_mode | cam_mode);	// ta_status;
	mms_set_noise_mode(info, N_TA);
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	if (buf[0] == 0x0E) { /* NOISE MODE */
		...
		info->noise_mode = 1;
		//mms_set_noise_mode(info, N_NOISE);
		goto out;
	}
...
//		if (gpio_get_value(GPIO_OLED_DET)) {
		if (gpio_get_value(PM8921_GPIO_PM_TO_SYS(GPIO_OLED_DET))) {
			dev_err(&client->dev, "[TSP] panel connected\n");
			...
			pr_err("[TSP] %s: tsp driver unload, lcd not connect\n", __func__);
			return ret;
		}
#endif
	}
...
pr_err( "[TSP] %s : %d \n", __func__,__LINE__);
dev_err(&client->dev,"[TSP] mms_ts_probe (%d)\n",__LINE__);
printk("[TSP] mms_ts_probe\n");
	...
	*/
	...
	ret = request_threaded_irq(client->irq, NULL, mms_ts_interrupt, IRQF_TRIGGER_LOW  | IRQF_ONESHOT, MELFAS_TS_NAME, info);
...
static int mms_ts_suspend(struct device *dev)
{
	...
	mms_set_noise_mode(info, N_CLEAR);
...
static void mms_ts_late_resume(struct early_suspend *h)
{
	struct mms_ts_info *info;
	info = container_of(h, struct mms_ts_info, early_suspend);
	gpio_tlmm_config(GPIO_CFG(info->pdata->gpio_int, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
