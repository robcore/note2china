...
int synaptics_rmi4_glove_mode_enables(struct synaptics_rmi4_data *rmi4_data)
{
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: [%02X]: %s\n",
		...
		(rmi4_data->glove_mode_enables == 0x06) ? "only clear cover enable" :
		(rmi4_data->glove_mode_enables == 0x05) ? "only fast glove enable" :
		(rmi4_data->glove_mode_enables == 0x03) ? "glove & clear cover enable" :
		(rmi4_data->glove_mode_enables == 0x02) ? "only clear cover enable" :
		(rmi4_data->glove_mode_enables == 0x01) ? "only glove enable" : "glove & clear cover disable");
