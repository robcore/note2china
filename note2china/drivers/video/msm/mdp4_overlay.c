...
#if 0 //defined(CONFIG_FB_MSM_CAMERA_CSC)
extern int csc_updated;
extern uint32_t csc_pre_bv_val;
...
void mdp4_overlay_vg_setup(struct mdp4_overlay_pipe *pipe)
{
	...
	if (csc_updated) {
		...
		uint32_t off;
		uint32_t base = (uint32_t) (vg_base + MDP4_VIDEO_CSC_OFF);
		off = ((uint32_t) base + CSC_MV_OFF);

		if (csc_pre_bv_val == 0x0000) { /* camera preview */
			...
			csc_array[9][0] = off+0x20;  csc_array[9][1] = csc_pre_bv_val;
		} else { /* video preview */
