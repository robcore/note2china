...
static int brightness_control(int bl_level)
{
	...
	*/
	...
	if (get_auto_brightness() == 6) {
		samsung_brightness_acl_ref[1] = 0x41; /*RE low, ACL on 30p*/
	} else {
		if (mipi_pd.acl_status || mipi_pd.siop_status)
			samsung_brightness_acl_ref[1] = 0x01; /*ACL on 30p*/
		else
...
static int acl_control(int bl_level)
{
	...
	/* 0x55 setting */
	if (get_auto_brightness() == 6) {
		samsung_brightness_acl_ref[1] = 0x41; /*ACL on 30p, re low*/
	} else {
		if (mipi_pd.acl_status || mipi_pd.siop_status)
			samsung_brightness_acl_ref[1] = 0x01; /*ACL on 30p*/
		else
