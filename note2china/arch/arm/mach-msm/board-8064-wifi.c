...
static int brcm_wlan_power(int onoff)
{
	...
	if (onoff) {
		...
		if (pm8xxx_gpio_config(GPIO_WL_REG_ON, &wl_en_on_cfg)) {		// yhcha-patch
		...
		/* if (gpio_direction_output(GPIO_WL_REG_ON, 0)) { */
		if (pm8xxx_gpio_config(GPIO_WL_REG_ON, &wl_en_off_cfg)) {		// yhcha-patch
