...
struct pm_gpio mdm2_gpio_off_param = {
...
struct pm_gpio mdm2_gpio_on_param = {
	.direction = PM_GPIO_DIR_OUT,
	.output_buffer = PM_GPIO_OUT_BUF_CMOS,
	.output_value = 1,
	.pull = PM_GPIO_PULL_NO,
...
static void mdm2_pmic_gpio_ctrl(int num, int onoff)
{
	int rc = 0;

	if (onoff == 1) {
		rc = pm8xxx_gpio_config(num, &mdm2_gpio_on_param);
		if (rc) {
			printk(KERN_INFO
				"gpio_config %d failed , rc=%d\n", num, rc);
		}
	} else if (onoff == 0) {
		rc = pm8xxx_gpio_config(num, &mdm2_gpio_off_param);
		if (rc) {
			printk(KERN_INFO
				"gpio_config %d failed , rc=%d\n", num, rc);
		}
...
static void mdm_toggle_soft_reset(struct mdm_modem_drv *mdm_drv)
{
	...
	pr_err("%s:id %d: soft_reset : %d\n",
		   __func__, mdm_drv->device_id, mdm_drv->pdata->soft_reset_inverted);

	if(mdm_drv->device_id ==0){
	...
	}else if(mdm_drv->device_id ==1){

		mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_soft_reset_gpio, 1);
		mdelay(10);
		mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_soft_reset_gpio, 0);

		mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_kpdpwr_n_gpio, 1);
		msleep(1000);
		mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_kpdpwr_n_gpio, 0);	
	}
}
...
static void mdm_power_down_common(struct mdm_modem_drv *mdm_drv)
{
	...
	mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_soft_reset_gpio, //HSLEE qsc en
...
static void mdm_do_first_power_on(struct mdm_modem_drv *mdm_drv)
{
	...
	if (0){//mdm_drv->pdata->kpd_not_inverted) { //temp
		kpd_direction_assert = 0;
		kpd_direction_de_assert = 1;
	}

	mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_soft_reset_gpio, 
					0);

	
	pr_err("%s:id %d: kpd : %d\n",
		   __func__, mdm_drv->device_id, mdm_drv->pdata->kpd_not_inverted);
	...
	mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_pmic_pwr_en_gpio,1);
	if (!mdm_drv->pdata->no_reset_on_first_powerup)
		mdm_toggle_soft_reset(mdm_drv);

	/* If the device has a kpd pwr gpio then toggle it. */
	if (GPIO_IS_VALID(mdm_drv->ap2qsc_kpdpwr_n_gpio)) { // HSLEE qsc en
		/* Pull AP2MDM_KPDPWR gpio high and wait for PS_HOLD to settle,
		 * then	pull it back low.
		 */
		pr_err("%s:id %d: Pulling AP2MDM_KPDPWR gpio high\n",
				 __func__, mdm_drv->device_id);
		mdm2_pmic_gpio_ctrl(mdm_drv->ap2qsc_kpdpwr_n_gpio, 
