...
void synaptics_power_onoff(bool enable)
{
	...
	if (!reg_l16) {
		...
		if (IS_ERR(reg_l16)) {
			printk(KERN_ERR "%s: could not get 8921_l16, rc = %ld\n",
					__func__, PTR_ERR(reg_l16));
			return;
		}
		ret = regulator_set_voltage(reg_l16, 3000000, 3000000);
		if (ret) {
			printk(KERN_ERR "%s: unable to set ldo17 voltage to 3.0V\n",
					__func__);
			return;
		}
	}
	...
	if (enable) {
		...
		if (!regulator_is_enabled(reg_l16)) {
			...
			if (ret)
				printk(KERN_ERR "%s: enable l16 failed, rc=%d\n",
						__func__, ret);
			else
				printk(KERN_INFO "%s: enable l16\n", __func__);
		}
#endif
		...
		if (regulator_is_enabled(reg_l16)) {
			...
			if (ret)
				printk(KERN_ERR "%s: disable l16 failed, rc=%d\n",
						__func__, ret);
			else
				printk(KERN_INFO "%s: disable l16\n", __func__);
		}
#endif
