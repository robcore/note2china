...
static int __init felica_init(void)
{
	...
	if(system_rev > 0 && system_rev < 15)
		pm8xxx_gpio_config(GPIO_FELICA_INTU, &nfc_irq_cfg);
	else
		gpio_tlmm_config(GPIO_CFG(GPIO_FELICA_INTU_REV06, GPIOMUX_FUNC_GPIO, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
{
	if (ice_gpiox_set(FPGA_GPIO_TDMB_EN, 0))
			pr_info("%s error in making FPGA_GPIO_TDMB_EN Low\n",
			__func__);

	if (ice_gpiox_set(FPGA_GPIO_TDMB_RST, 0))
			pr_info("%s error in making FPGA_GPIO_TDMB_RST Low\n",
			__func__);
}
static void tdmb_gpio_on(void)
{
	...
	if (ice_gpiox_set(FPGA_GPIO_TDMB_EN, 1))
			pr_info("%s error in making FPGA_GPIO_TDMB_EN High\n",
			__func__);
	...
	if (ice_gpiox_set(FPGA_GPIO_TDMB_RST, 1))
			pr_info("%s error in making FPGA_GPIO_TDMB_RST High\n",
			__func__);
	usleep_range(10000, 10000);
...
static int __init tdmb_dev_init(void)
{
	...
	tdmb_pdata.irq = PM8921_GPIO_IRQ(PM8921_IRQ_BASE, 33),
