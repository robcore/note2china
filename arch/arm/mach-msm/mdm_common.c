...
#define GET_QSC_MDM_RAM_DUMP_DONE 2
static int qsc_mdm_dump_done = 0;
#endif
...
struct pm_gpio mdm_gpio_off_param = {
...
struct pm_gpio mdm_gpio_on_param = {
	.direction = PM_GPIO_DIR_OUT,
	.output_buffer = PM_GPIO_OUT_BUF_CMOS,
	.output_value = 1,
	.pull = PM_GPIO_PULL_NO,
...
static void mdm_pmic_gpio_ctrl(int num, int onoff)
{
	int rc = 0;

	if (onoff == 1) {
		rc = pm8xxx_gpio_config(num, &mdm_gpio_on_param);
		if (rc) {
			printk(KERN_INFO
				"gpio_config %d failed , rc=%d\n", num, rc);
		}
	} else if (onoff == 0) {
		rc = pm8xxx_gpio_config(num, &mdm_gpio_off_param);
		if (rc) {
			printk(KERN_INFO
				"gpio_config %d failed , rc=%d\n", num, rc);
		}
				...
				unsigned long arg)
{
...
#ifdef CONFIG_SEC_DEBUG_MDM_FILE_INFO
			if (sec_debug_is_enabled() && (qsc_mdm_dump_done == GET_QSC_MDM_RAM_DUMP_DONE )) {
	...
	case GET_DLOAD_STATUS:
		...
		break;
	case IMAGE_UPGRADE:
		cancel_delayed_work(&mdev->mdm2ap_status_check_work);	// cancel status check, pje
		pr_err("%s Image upgrade ioctl recieved\n", __func__);
						...
						struct mdm_device *mdev)
{
	...
	pres = platform_get_resource_byname(pdev, IORESOURCE_IO,
							"AP2QSC_SOFT_RESET");
	mdm_drv->ap2qsc_soft_reset_gpio = pres ? pres->start : -1;

	/* AP2QSC_KPDPWR_N */
	pres = platform_get_resource_byname(pdev, IORESOURCE_IO,
							"AP2QSC_KPDPWR_N");
	mdm_drv->ap2qsc_kpdpwr_n_gpio = pres ? pres->start : -1;

	/* AP2QSC_PMIC_PWR_EN */ 
	pres = platform_get_resource_byname(pdev, IORESOURCE_IO,
							"AP2QSC_PMIC_PWR_EN");
	mdm_drv->ap2qsc_pmic_pwr_en_gpio = pres ? pres->start : -1;
...
//	if (GPIO_IS_VALID(mdm_drv->ap2qsc_kpdpwr_n_gpio)) 
//		gpio_free(mdm_drv->ap2qsc_kpdpwr_n_gpio);
//	if (GPIO_IS_VALID(mdm_drv->ap2qsc_pmic_pwr_en_gpio)) 
//		gpio_free(mdm_drv->ap2qsc_pmic_pwr_en_gpio);
...
//	if (GPIO_IS_VALID(mdm_drv->ap2qsc_kpdpwr_n_gpio)) 
//		gpio_request(mdm_drv->ap2qsc_kpdpwr_n_gpio, "AP2QSC_KPDPWR_N");
...
//	if (GPIO_IS_VALID(mdm_drv->ap2qsc_pmic_pwr_en_gpio)) 
//		gpio_request(mdm_drv->ap2qsc_pmic_pwr_en_gpio,
//					 "AP2QSC_PMIC_PWR_EN");
//	if (GPIO_IS_VALID(mdm_drv->ap2mdm_soft_reset_gpio))
...
pblrdy_err:
	 ...
	 */
	if (GPIO_IS_VALID(mdm_drv->ap2qsc_pmic_pwr_en_gpio))
		mdm_pmic_gpio_ctrl(mdm_drv->ap2qsc_pmic_pwr_en_gpio, 1);
	mdm_pmic_gpio_ctrl(mdm_drv->ap2qsc_soft_reset_gpio, 0);
...
static void mdm_modem_shutdown(struct platform_device *pdev)
{
	...
	mdm_ops->power_down_mdm_cb(mdm_drv);
	if (GPIO_IS_VALID(mdm_drv->ap2qsc_pmic_pwr_en_gpio)) 
		mdm_pmic_gpio_ctrl(mdm_drv->ap2qsc_pmic_pwr_en_gpio, 0);
}
