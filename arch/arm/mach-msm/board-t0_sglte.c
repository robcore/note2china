...
/* PCIe pmic gpios */
#ifndef CONFIG_MACH_JF_SGLTE
#define PCIE_WAKE_N_PMIC_GPIO 12
#endif
#define PCIE_PWR_EN_PMIC_GPIO 13
#define PCIE_RST_N_PMIC_MPP 1

#ifdef CONFIG_MACH_JF_SGLTE
#define QSC_PWR_EN_PMIC_GPIO 	PM8921_GPIO_PM_TO_SYS(8)
#define QSC_SOFT_RST_PMIC_GPIO 	PM8921_GPIO_PM_TO_SYS(12)
#define QSC_KPD_PWR_PMIC_GPIO PM8921_GPIO_PM_TO_SYS(16)

struct pm_gpio gpio_qsc_param_high = {
...
struct pm_gpio tflash_level_on_cfg = {
   		.direction = PM_GPIO_DIR_OUT,
		.output_value = 1,
...
struct pm_gpio tflash_level_off_cfg = {
   		.direction = PM_GPIO_DIR_OUT,
		.output_value = 0,
		.pull = PM_GPIO_PULL_DN,
		.vin_sel = 2,
...
void tflash_ls_en_gpio_contorl(bool on)
{
	if (!on)
		pm8xxx_gpio_config(GPIO_TFLASH_LS_EN,&tflash_level_off_cfg);
	else
		pm8xxx_gpio_config(GPIO_TFLASH_LS_EN,&tflash_level_on_cfg);
}
#endif
...
#if defined(CONFIG_MACH_JF_SGLTE)
	0x2F, 0x81, /* update DC voltage level */
#else
...
{
#ifdef CONFIG_MACH_JF_SGLTE
	*acc = K330_TOP_RIGHT_LOWER;
	*mag = YAS532_BOTTOM_RIGHT_UPPER;
#else
...
static void barcode_gpio_config(void)
{
	...
	if (system_rev >= BOARD_REV15) {
		gpio_tlmm_config(GPIO_CFG(72, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);	// CLK
		gpio_tlmm_config(GPIO_CFG(77, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);	// SI
	}else
#endif	
	{
		pm8xxx_mpp_config(PM8921_MPP_PM_TO_SYS(PMIC_MPP_FPGA_SPI_CLK), &barcode_mppcfg);
		pm8xxx_mpp_config(PM8921_MPP_PM_TO_SYS(PMIC_MPP_FPGA_SPI_SI), &barcode_mppcfg);
...
#if 0 // this not use in T0-SGLTE jjlee
...
static struct mdm_platform_data sglte2_qsc_platform_data = {
	...
	.no_a2m_errfatal_on_ssr = 1,
#ifndef SGLTE_QSC_RESET // not using 19520 patch			
...
struct platform_device hvca_i2c_gpio_device = {
	.name = "i2c-gpio",
	.id = 27,//MSM_CAMERA_SW_I2C_BUS_ID, // 27
	.dev = {
...
static struct i2c_registry apq8064_i2c_devices[] __initdata = {
...
#if defined(CONFIG_VIDEO_MHL_V1) || defined(CONFIG_VIDEO_MHL_V2)
	{
		I2C_SURF | I2C_FFA | I2C_FLUID,
#ifdef CONFIG_MACH_JF_SGLTE	
		I2C_LEDS_BUS_ID, //kk0704.park :: Qualcomm Guide for I2C communication
#else
		MSM_MHL_I2C_BUS_ID,
#endif
		mhl_i2c_board_info,
...
#ifdef CONFIG_SEC_FPGA
#if defined(CONFIG_MACH_JF_SGLTE)
	pr_err("%s : system_rev = %d/ %d\n", __func__,system_rev, BOARD_REV15);

	if (system_rev >= BOARD_REV15) {
		barcode_i2c_gpio_data.sda_pin = 77;	// SI
		barcode_i2c_gpio_data.scl_pin = 72;	// CLK
	}else
#endif	
	{
...
#if defined(CONFIG_MACH_JF_SGLTE)
	if (system_rev >= BOARD_REV15) {
		barcode_emul_info.spi_clk = 72;	// CLK
		barcode_emul_info.spi_si  = 77;	// SI
	}else
#endif
	{
...
void vreg_l14_onoff(int onoff)
{
	...
	if (vreg_l14 == NULL) {
		vreg_l14 = regulator_get(NULL, "8921_l14");
		if (IS_ERR(vreg_l14))
			return ;

		ret = regulator_set_voltage(vreg_l14, 1800000, 1800000);
		if (ret)
			pr_err("%s: error vreg_l14 setting voltage ret=%d\n",
				__func__, ret);
	}
	if(onoff)
		ret = regulator_enable(vreg_l14);
	else
		ret = regulator_disable(vreg_l14);

	if (ret)
		pr_err("%s: error vreg_l14 en/disable ret=%d\n",
			__func__, ret);
}
EXPORT_SYMBOL(vreg_l14_onoff);
#endif
...
static void __init apq8064_common_init(void)
{
	...
	printk(KERN_DEBUG"[TSP] CONFIG_TOUCHSCREEN_MELFAS_NOTE\n");
	{
		extern void __init midas_tsp_init(void);
		midas_tsp_init();
	}
#endif
#if defined(CONFIG_INPUT_WACOM)
	{
		extern void __init midas_wacom_init(void);
		midas_wacom_init();
	}
#endif
