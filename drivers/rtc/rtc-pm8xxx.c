...
static struct rtc_wkalrm sec_pwronalarm_time;
#endif
...
#ifdef CONFIG_RTC_AUTO_PWRON
static int
pm8058_rtc0_getbootalarm(struct device *dev, struct rtc_wkalrm *a)
{
	struct rtc_time b; 
	int ret = 0; 
...
static int
pm8058_rtc0_setbootalarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	if (!alarm->enabled) {
		...
		rtc_dd->ctrl_reg = ctrl_reg;
		sec_pwronalarm_time.enabled = 0;  // disable pwr on alarm to prevent retrying
	}
	else
	{
		...
		if ( alarm != &sec_pwronalarm_time ) {
			memcpy(&sec_pwronalarm_time, alarm, sizeof(struct rtc_wkalrm));
		}
	}
	...
	pr_info( "[%s] READ BOOT ALARM : %d-%d-%d %d:%d:%d\n", __func__ ,
			alarm->time.tm_year, alarm->time.tm_mon, alarm->time.tm_mday,
			alarm->time.tm_hour, alarm->time.tm_min, alarm->time.tm_sec);
...
{
	pr_info("[%s] enable=%d\n",__func__, sec_pwronalarm_time.enabled);
	return pm8058_rtc0_setbootalarm(dev, &sec_pwronalarm_time);
}
...
#ifdef CONFIG_RTC_AUTO_PWRON
	.read_bootalarm = pm8058_rtc0_getbootalarm,
	.set_bootalarm  = pm8058_rtc0_setbootalarm,
#endif /*CONFIG_RTC_AUTO_PWRON*/
...
static int __devinit pm8xxx_rtc_probe(struct platform_device *pdev)
{
	...
	if((secs < 86400) || (tm.tm_year < 100) || (tm.tm_year > 136) )
	{
		tm.tm_year = 113;
		tm.tm_mon = 0;
		tm.tm_mday = 1;
		...
		tm.tm_min = 0;
		tm.tm_hour = 0;
		pr_info("%s [RTC] RTC has default time. So it will be 2013-01-01 \n",__func__);
