               ...
               struct device_attribute *attr, const char *buf, size_t count)
{
	...
	cscctrl.mode = mode;
	if (cscctrl.mode == 1)
	       csc_pre_bv_val = 0xfff0;
	else {
	       csc_pre_bv_val = 0x0000;
	}
	csc_updated = 1;

	mdp_csc_convert[1].csc_pre_bv[0] = csc_pre_bv_val;
	pr_info("%s: csc ctrl set to %d val:0x%4X\n", __func__, mode, mdp_csc_convert[1].csc_pre_bv[0]);
...
void mdp4_hw_init(void)
{
	...
	mdp_bus_scale_restore_request(); //flashing blue screen
