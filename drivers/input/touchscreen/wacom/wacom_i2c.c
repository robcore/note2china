...
struct delayed_work work_wacom_q;
...
#if 1 // defined(CONFIG_MACH_Q1_BD) || defined(CONFIG_MACH_T0)
int wacom_i2c_firm_update(struct wacom_i2c *wac_i2c)
{
	int ret;
	int retry = 2; 		//  jjlee , need changed after Rev02 ,   3;
	...
	while (retry--) {
		...
		if (ret == 0) {
			printk(KERN_ERR "[E-PEN] firmup break, ret=(%d)\n", ret);
...
static void wacom_i2c_early_suspend(struct early_suspend *h)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_PEN_PDCT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), 1);
	gpio_tlmm_config(GPIO_CFG(GPIO_PEN_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), 1);
}
...
static void wacom_i2c_late_resume(struct early_suspend *h)
{
	...
	gpio_tlmm_config(GPIO_CFG(GPIO_PEN_PDCT, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	gpio_tlmm_config(GPIO_CFG(GPIO_PEN_IRQ, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
					 ...
					 char *buf)
{
	...
	printk(KERN_DEBUG "[E-PEN] pen_insert_check : %d\n",wac_i2c->pen_insert);
	return sprintf(buf, "%s\n", wac_i2c->pen_insert ? "OK" : "NG");
#else
	printk(KERN_DEBUG "[E-PEN] connection_check : %d\n", wac_i2c->connection_check);
...
#if defined(CONFIG_MACH_JF_SGLTE)
extern unsigned int fpga_init_fail;
#endif
			   ...
			   const struct i2c_device_id *id)
{
	...
	int ret = 0;
#if 1 // defined(CONFIG_MACH_T0)
	int digitizer_type = 3;
#endif

	printk(KERN_DEBUG"[E-PEN] %s star\n", __func__);
...
#if defined(CONFIG_MACH_JF_SGLTE)
	if(fpga_init_fail){
		printk(KERN_ERR "[E-PEN] FPGA init fail, %d\n", fpga_init_fail);
		goto err_i2c_fail;
	}
#endif
	...
	if (wac_i2c->irq_flag) {
		...
		if (ret < 0) {
			printk(KERN_ERR
			       "[E-PEN] failed to request irq(%d) - %d, line:%d\n",
			       wac_i2c->irq, ret, __LINE__);
...
//__define_initcall("10", wacom_i2c_init, 10);
