		...
		const struct i2c_device_id *id)
{
	...
	if (system_rev >= 15){ //BOARD_REV15) {
		gpio_tlmm_config(GPIO_CFG(72, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);	// CLK
		gpio_tlmm_config(GPIO_CFG(77, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);	// SI
	}
#else
	data->firmware_dl =
